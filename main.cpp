...
void wsReceiveCallback( char *payloadData ) {
  ...
  char bufferr[10];
  char a[15];
  int num = mesh.connectionCount( NULL ), i=0;
  ...
  strcat(bufferr, "");
  strcat(data, ";");
  int chipidValue = control["0"][1];
  if (chipidValue == 1){
    digitalWrite(LED, HIGH);
    }else{
    digitalWrite(LED, LOW);            
  }
  sprintf(bufferr, "%d", chipidValue);
  strcat(data, bufferr);
  strcat(bufferr, "");
  strcat(data, ";");
  for(i=1;i<=num; i ++){
      ...
      strcat(data, ";");
      sprintf(a, "%d", i);
      int values = control[a][1];
      sprintf(bufferr, "%d", values);
      strcat(data, bufferr);
      strcat(data, ";");
      strcat(bufferr, "");
  }
